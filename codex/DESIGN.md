...
- **OV session** — `viking://user/sessions/<user>-codex-<codex-session-id>`. New captures
  derive the OV session id from the codex `session_id` with the `<user>-codex-` prefix,
...
same `<user>-codex-<codex-session-id>` OV session id for the post-compact half.
...
(or no state file remains), the hook derives `<user>-codex-<codex-session-id>`,
...
re-derives the same `<user>-codex-<codex-session-id>` OV session id and starts
...
```json
{
  ...
  "ovSessionId": "user01-codex-0193af...-or-null", // null means "committed, awaiting next Stop"
