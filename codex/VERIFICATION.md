...
export PLUGIN=/path/to/openviking-plugins/codex
...
Expect: `{"systemMessage":"appended 2 turn(s) to OpenViking session tester-codex-verify-sess"}`.

State file:
```bash
cat $STATE_DIR/state/verify-sess.json
# {"codexSessionId":"verify-sess","ovSessionId":"tester-codex-verify-sess","capturedTurnCount":2,...}
```

OV side:
```bash
OPENVIKING_CONFIG_FILE=$OV_CONF ov read viking://user/sessions/tester-codex-verify-sess/messages.jsonl
...
`viking://user/sessions/tester-codex-verify-sess/messages.jsonl` — 4 records now.
...
Expect: `OpenViking session tester-codex-verify-sess is committed`.

State file: `ovSessionId` is now `null`, `capturedTurnCount` stays at 4.

OV side:
```bash
OPENVIKING_CONFIG_FILE=$OV_CONF ov ls viking://user/sessions/tester-codex-verify-sess
# messages.jsonl is now size 0 (archived)
# history/archive_001/ exists with the committed messages
OPENVIKING_CONFIG_FILE=$OV_CONF ov read viking://user/sessions/tester-codex-verify-sess/history/archive_001/messages.jsonl
```
...
Expect: `appended 2 turn(s) to OpenViking session tester-codex-verify-sess`.
...
# Expect with an existing archive for user01-codex-any: hookSpecificOutput.additionalContext
# containing "OpenViking session archive digest" and a viking://user/sessions/user01-codex-any/history/ URI.
```
