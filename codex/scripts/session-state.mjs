 ...
 * OV session id is derived as `<user>-codex-<codex-session-id>` for new
 * captures.
...
export function deriveOvSessionId(codexSessionId) {
  const user = process.env.OPENVIKING_USER || process.env.OPENVIKING_ACCOUNT || "codex";
  return `${safeId(user)}-codex-${safeId(codexSessionId || "unknown")}`;
}

export function resolveOvSessionId(state) {
  // Keep legacy persisted UUIDs so already-captured turns are not orphaned
  // before their next commit. Fresh or cleared state derives the
  // <user>-codex-* id.
