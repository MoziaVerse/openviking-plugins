 ...
 *
 * Format:
 *   parent:    <user>-claude-<ccSessionId>
 *   subagent:  <user>-claude-<ccSessionId>__<subagentId>
 *
...
function safeId(value, fallback) {
  ...
  return raw.replace(/[^A-Za-z0-9._-]/g, "_") || String(fallback || "claude");
}

function readOvcliIdentity() {
  const file = resolvePath(
    (process.env.OPENVIKING_CLI_CONFIG_FILE || join(homedir(), ".openviking", "ovcli.conf"))
      .replace(/^~/, homedir()),
  );
  try {
    const cfg = JSON.parse(readFileSync(file, "utf-8"));
    return cfg.user || cfg.account || "";
  } catch {
    return "";
  }
}

function currentUserSegment() {
  return safeId(
 ...
 *
 * Optionally append a suffix (e.g. subagent_id) for session isolation. The
 * suffix is normalized, and a legacy `subagent:` prefix is stripped. Result:
 * `<user>-claude-<ccSessionId>__<suffix>`.
 */
export function deriveOvSessionId(ccSessionId, suffix = "") {
  ...
  const base = `${currentUserSegment()}-claude-${safeId(ccSessionId, "unknown")}`;
  if (!suffix) return base;
  const normalized = safeId(String(suffix).replace(/^subagent:/, ""), "subagent");
